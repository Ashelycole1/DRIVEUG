    ...
    <div className="flex flex-col gap-6 px-4 md:px-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">Explore and filter premium rental cars in Uganda</p>
        </div>
        ...
        <div className="relative max-w-md w-full">
          ...
          <input
            ...
            className="w-full bg-white border border-gray-200 rounded-2xl pl-12 pr-4 py-3 text-sm focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all shadow-sm"
          />
          {searchQuery && (
            <button
      ...
      <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6 sticky top-24">
            ...
            <div className="space-y-3 pt-4 border-t border-gray-100">
              <div className="flex justify-between items-center text-sm">
                <span className="font-semibold text-gray-700">Max Daily Rate</span>
            ...
            {/* Fuel Type */}
            <div className="space-y-3 pt-4 border-t border-gray-100">
              <h3 className="font-semibold text-sm text-gray-700">Fuel Type</h3>
            ...
            <div className="space-y-3 pt-4 border-t border-gray-100">
              <h3 className="font-semibold text-sm text-gray-700">Vehicle Class</h3>
            ...
            {/* Driver Options */}
            <div className="space-y-3 pt-4 border-t border-gray-100">
              <h3 className="font-semibold text-sm text-gray-700">Driver Options</h3>
              <div className="space-y-2">
                <label className="flex items-center gap-3 cursor-pointer text-sm text-gray-600 hover:text-gray-900 transition">
                  <input
                    type="checkbox"
        ...
        <div className="flex-1 space-y-6">
          ...
          {filteredCars.length > 0 ? (
            ...
            <div className="bg-white rounded-3xl p-12 border border-gray-100 text-center max-w-xl mx-auto space-y-4 shadow-sm mt-8">
              <p className="text-lg font-bold text-gray-800">No vehicles match your criteria</p>
              <p className="text-gray-500 text-sm">
                Try loosening your filters, adjusting the pricing range, or searching for other keywords.
              </p>
              <button
                onClick={resetFilters}
                className="px-6 py-2.5 bg-gray-900 hover:bg-gray-850 text-white rounded-full text-sm font-semibold transition"
              >
                Clear All Filters
              </button>
            </div>
...
function CarCard({ car }) {
  return (
    <Link
      to={`/cars/${car.id}`}
      className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 flex flex-col group hover:shadow-md hover:border-gray-200 transition-all duration-300"
    >
      {/* Image */}
      <div className="h-44 overflow-hidden relative bg-gray-50">
        ...
        <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-lg flex items-center gap-1 shadow-sm">
          ...
          <span className="text-[11px] font-bold text-gray-800">{car.rating_avg}</span>
        </div>
      ...
      <div className="p-4 flex flex-col gap-3">
        ...
        {car.driver_mode === 'driver_required' && (
          <div className="flex items-center justify-between bg-amber-50 border border-amber-100 rounded-lg px-3 py-1.5">
            <span className="text-[11px] font-medium text-amber-700">Chauffeur included</span>
            <span className="text-[10px] font-semibold text-amber-800">+UGX {car.driver_fee_ugx?.toLocaleString()}/d</span>
          </div>
        )}
        {car.driver_mode === 'driver_available' && (
          <div className="flex items-center justify-between bg-gray-50 border border-gray-100 rounded-lg px-3 py-1.5">
            <span className="text-[11px] font-medium text-gray-600">Chauffeur optional</span>
            <span className="text-[10px] font-semibold text-gray-500">+UGX {car.driver_fee_ugx?.toLocaleString()}/d</span>
          </div>
        )}
        {car.driver_mode === 'self_drive_only' && (
          <div className="bg-gray-50 border border-gray-100 rounded-lg px-3 py-1.5">
            <span className="text-[11px] font-medium text-gray-500">Self Drive Only</span>
          </div>
        )}

        {/* Specs row */}
        <div className="flex items-center justify-between text-gray-400 text-[11px] pt-1 border-t border-gray-100">
        ...
        <div className="flex items-baseline justify-between pt-1 border-t border-gray-100">
          ...
          <div>
            <span className="font-bold text-gray-900 text-sm">UGX {car.daily_price_ugx.toLocaleString()}</span>

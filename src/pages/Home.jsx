    ...
    <div className="flex flex-col gap-10 pb-16">
      ...
      <section className="relative -mt-8 rounded-b-3xl md:rounded-3xl overflow-hidden h-[420px] shadow-md">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center text-white">
          ...
          {/* Search bar */}
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-2 flex items-center w-full max-w-xl shadow-2xl gap-2">
            <div className="flex-1 flex items-center px-3 gap-2.5">
              ...
              <input 
                ...
                className="w-full bg-transparent outline-none text-gray-800 text-sm placeholder-gray-400 font-normal"
              />
            </div>
            <button 
              onClick={() => navigate(`/cars?q=${search}`)}
              className="bg-gray-900 hover:bg-gray-800 text-white px-6 py-3 rounded-xl transition-all duration-200 font-semibold text-sm active:scale-95 shrink-0"
            >
              Search
            </button>
          </div>
        ...
        <div className="flex flex-wrap gap-2.5 justify-center">
          {categories.map((cat, i) => (
            <Link 
              ...
              className="px-5 py-2.5 bg-white border border-gray-200 hover:border-gray-500 hover:bg-gray-50 rounded-xl text-xs font-semibold text-gray-600 hover:text-gray-900 shadow-sm transition-all duration-200"
            >
              {cat.label}
            </Link>
          ))}
      ...
      <section className="px-4 md:px-0">
        <div className="flex justify-between items-center mb-5">
          <div>
            <h2 className="text-xl font-bold text-gray-900 tracking-tight">Featured Fleet</h2>
...
function CarCard({ car }) {
  return (
    <Link 
      to={`/cars/${car.id}`} 
      className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md hover:border-gray-200 transition-all duration-300 flex flex-col group"
    >
      {/* Image */}
      ...
      <div className="p-4 flex flex-col flex-1 gap-3">
        ...
        <div className="flex items-baseline justify-between pt-3 border-t border-gray-100 mt-auto">
